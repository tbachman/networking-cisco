...
if $(python -c "import neutron" 2> /dev/null); then
    ...
    (cd /tmp/openstack/neutron && git checkout stable/kilo)
    pip install /tmp/openstack/neutron
else
    # Download or update neutron-master tarball and install
    ( cd .test-tars && wget -N http://tarballs.openstack.org/neutron/neutron-stable-kilo.tar.gz )
    pip install .test-tars/neutron-stable-kilo.tar.gz
fi
